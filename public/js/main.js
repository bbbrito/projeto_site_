;(function () {
	...
	var burgerMenu = function() {
		...
		$('body').on('click', '.js-faqui-nav-toggle', function(event){
	...
	var clickMenu = function() {
		...
		$('a:not([class="external"])').click(function(event){
		    ...
		    if ( navbar.is(':visible')) {
		    	...
		    	$('.js-faqui-nav-toggle').removeClass('active');
		    }
	...
	var windowScroll = function() {
		...
		$(window).scroll(function(event){
		   	...
		   	var header = $('#faqui-header'),
				scrlTop = $(this).scrollTop();

			if ( scrlTop > 500 && scrlTop <= 2000 ) {
				header.addClass('navbar-fixed-top faqui-animated slideInDown');
			} else if ( scrlTop <= 500) {
				if ( header.hasClass('navbar-fixed-top') ) {
					header.addClass('navbar-fixed-top faqui-animated slideOutUp');
					setTimeout(function(){
						header.removeClass('navbar-fixed-top faqui-animated slideInDown slideOutUp');
					}, 100 );
				}
			} 
	...
	var servicesAnimate = function() {
		...
		if ( $('#faqui-our-services').length > 0 ) {	
			$('#faqui-our-services .to-animate').each(function( k ) {
	...
	var servicesWayPoint = function() {
		...
		if ( $('#faqui-our-services').length > 0 ) {
			$('#faqui-our-services').waypoint( function( direction ) {
	...
	var featuresAnimate = function() {
		...
		if ( $('#faqui-features').length > 0 ) {	
			$('#faqui-features .to-animate').each(function( k ) {
	...
	var featuresWayPoint = function() {
		...
		if ( $('#faqui-features').length > 0 ) {
			$('#faqui-features').waypoint( function( direction ) {
	...
	var testimonialsAnimate = function() {
		...
		if ( $('#faqui-testimonials').length > 0 ) {	
			$('#faqui-testimonials .to-animate').each(function( k ) {
	...
	var testimonialsWayPoint = function() {
		...
		if ( $('#faqui-testimonials').length > 0 ) {
			$('#faqui-testimonials').waypoint( function( direction ) {
	...
	var pricingAnimate = function() {
		...
		if ( $('#faqui-pricing').length > 0 ) {	
			$('#faqui-pricing .to-animate').each(function( k ) {
	...
	var pricingWayPoint = function() {
		...
		if ( $('#faqui-pricing').length > 0 ) {
			$('#faqui-pricing').waypoint( function( direction ) {
	...
	var pressAnimate = function() {
		...
		if ( $('#faqui-press').length > 0 ) {	
			$('#faqui-press .to-animate').each(function( k ) {
	...
	var pressWayPoint = function() {
		...
		if ( $('#faqui-press').length > 0 ) {
			$('#faqui-press').waypoint( function( direction ) {
